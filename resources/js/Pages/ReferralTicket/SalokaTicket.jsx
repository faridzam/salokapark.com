...
export function useIsMounted() {
    ...
    const [referrer, setReferrer] = React.useState(false);
    React.useEffect(() => {
        ...
        if(referrerParam){
            // setReferrer(encryptStorage.decryptValue(decodeURIComponent(sessionCodeIDParams)));
            // sessionStorage.setItem('referrer', referrerParam);
            encryptStorage.setItem('referrer', JSON.stringify(referrerParam));
        } else {
...
export default function Ticket(props) {
    ...
    const getTicketByDate = (date) => {
        axios.post('/api/get-ticket-date-referral', {
            date: date
        }).then((response) => {
                                                    ...
                                                    sx={{
                                                        ...
                                                        sx={{
                                                            ...
                                                            {totalBill === 0 || bookingDate <= today || minQtyReq === false
                                                                ...
                                                                :   <Link
                                                                    href={route('dataPemesanReferral')}
                                                                    style={{
                                                                        height: '100%'
                                                                    }}
                                                                    >
